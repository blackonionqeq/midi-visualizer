...
Repository-wide instructions for coding agents. Keep changes consistent with the architecture and conventions below.
...
Any speed-related change must preserve this bidirectional scaling; otherwise audio and visuals will drift.
...
- **@tonejs/midi dual module**: in the browser/Vite side `import { Midi }` works fine; Node scripts must use the CommonJS-compatible form `import pkg from '@tonejs/midi'; const { Midi } = pkg` (see `scripts/*.mjs`).
